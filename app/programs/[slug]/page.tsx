    ...
    <div className="public-page">
      ...
      <main className="public-page__main">
        <section className="program-detail-hero">
          <div className="shell program-detail-hero__grid">
            <div className="program-detail-visual" aria-hidden="true">
              <span className="program-detail-visual__navy" /><span className="program-detail-visual__coral" /><span className="program-detail-visual__gold" />
              <strong>{program.featured ? "آینه" : "رشد"}</strong><p>{program.title}</p>
            </div>
            <div className="program-detail-copy">
              <p className="eyebrow">جزئیات برنامه</p>
              <h1>{program.title}</h1>
              <p>{program.summary || "اطلاعات تکمیلی این برنامه به‌زودی منتشر می‌شود."}</p>
              <div className="program-detail-actions">
                ...
                <div className="program-detail-buttons">
                  <a className="button button--primary" href={registrationHref}>درخواست حضور</a>
        ...
        <section className="program-overview">
          <div className="shell">
            <div className="section-intro section-intro--compact"><p className="eyebrow">درباره برنامه</p><h2>از ایده تا یک مسیر رشد قابل اندازه‌گیری</h2></div>
            <div className="program-overview-grid">
              <article className="program-goal"><p className="eyebrow">هدف برنامه</p><h3>ساختن شواهد واقعی برای رشد</h3><p>{program.outputs || program.summary || "خروجی‌های دقیق برنامه در پنل مدیریت قابل تکمیل است."}</p></article>
              <div className="program-metrics">
                <article className="program-metric"><strong>{program.duration || "—"}</strong><span>مدت برنامه</span></article>
                <article className="program-metric"><strong>{formatPersianDate(program.startsAt)}</strong><span>شروع</span></article>
                <article className="program-metric"><strong>{formatPersianDate(program.endsAt)}</strong><span>پایان</span></article>
              </div>
            </div>
          </div>
        </section>
